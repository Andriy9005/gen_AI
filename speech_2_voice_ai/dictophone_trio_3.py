...
def correct_text(text):
    ...
    try:
        ...
        print(f"⚠️ Помилка при перевірці тексту: {e}")
...
try:
    ...
    print(f"❌ Невдале виконання correct_text: {e}")
    corrected = raw_text  # все одно продовжуємо
...
import tokenizer  # 👈 обовʼязково для кастомного токенайзера
...
# 🔁 Транслітерація українського тексту для FastPitch-моделі
...
def synthesize_and_play(text, path, lang_code="en"):
    if lang_code == "en":
        ...
        speaker = 0  # 👈 необхідно для мультіспікерної моделі
        text = transliterate_uk(text)
    ...
    with torch.no_grad():
        ...
        if speaker is not None:
            spectrogram = fastpitch.generate_spectrogram(tokens=parsed, speaker=speaker)
        else:
